...
pub trait Semantics: Sized {
    ...
    fn from_bits(bits: u256) -> IeeeFloat<Self> {
        ...
        let mut r = IeeeFloat {
            sig: [bits & ((Limb::ONE << (Self::PRECISION - 1)) - Limb::ONE)],
            // Convert the exponent from its bias representation to a signed integer.
            exp: (exponent.low_u64() as ExpInt) - Self::MAX_EXP,
            category: Category::Zero,
            sign: sign != Limb::ZERO,
            marker: PhantomData,
        };

        if r.exp == Self::MIN_EXP - 1 && r.sig == [Limb::ZERO] {
            // Exponent, significand meaningless.
            r.category = Category::Zero;
        } else if r.exp == Self::MAX_EXP + 1 && r.sig == [Limb::ZERO] {
            // Exponent, significand meaningless.
    ...
    fn to_bits(x: IeeeFloat<Self>) -> u256 {
        ...
        let mut significand = x.sig[0] & ((Limb::ONE << (Self::PRECISION - 1)) - Limb::ONE);
        ...
        let exponent = u256::from((exponent + Self::MAX_EXP) as u64);

        (Limb::from(x.sign) << (Self::BITS - 1)) | (exponent << (Self::PRECISION - 1)) | significand
    }
}
...
impl Semantics for X87DoubleExtendedS {
    ...
    fn from_bits(bits: u256) -> IeeeFloat<Self> {
        ...
        if r.exp == Self::MIN_EXP - 1 && r.sig == [Limb::ZERO] {
            // Exponent, significand meaningless.
            r.category = Category::Zero;
        } else if r.exp == Self::MAX_EXP + 1 && r.sig == [Limb::ONE << (Self::PRECISION - 1)] {
            // Exponent, significand meaningless.
            r.category = Category::Infinity;
        } else if r.exp == Self::MAX_EXP + 1 && r.sig != [Limb::ONE << (Self::PRECISION - 1)] {
            // Sign, exponent, significand meaningless.
    ...
    fn to_bits(x: IeeeFloat<Self>) -> u256 {
        ...
        let integer_bit = sig::get_bit(&x.sig, Self::PRECISION - 1);
        let mut significand = x.sig[0] & ((Limb::ONE << Self::PRECISION) - Limb::ONE);
        let exponent = match x.category {
            Category::Normal => {
                if x.exp == Self::MIN_EXP && !integer_bit {
                    // Denormal.
            ...
            Category::Infinity => {
                // FIXME(eddyb) Maybe we should guarantee an invariant instead?
                significand = Limb::ONE << (Self::PRECISION - 1);
                Self::MAX_EXP + 1
            }
            Category::NaN => Self::MAX_EXP + 1,
        };
        ...
        (u256::from(x.sign) << (Self::BITS - 1)) | (exponent << Self::PRECISION) | significand
    }
}
...
impl<S: Semantics> Float for IeeeFloat<S> {
    ...
    fn snan(payload: Option<u256>) -> Self {
        ...
        if snan.sig[0] & !S::QNAN_SIGNIFICAND == Limb::ZERO {
    ...
    fn largest() -> Self {
        ...
        //   significand = 1..1
        IeeeFloat {
            sig: [(Limb::ONE << S::PRECISION) - Limb::ONE],
            exp: S::MAX_EXP,
    ...
    fn mul_add_r(mut self, multiplicand: Self, addend: Self, round: Round) -> StatusAnd<Self> {
        ...
        if addend.is_non_zero() {
            ...
            let mut ext_addend_sig = [addend.sig[0], Limb::ZERO];
    ...
    fn round_to_integral(self, round: Round) -> StatusAnd<Self> {
        ...
        // addition instead.
        assert!(S::PRECISION <= 256);
        let mut status;
        let magic_const = unpack!(status=, Self::from_u256(u256::ONE << (S::PRECISION - 1)));
    ...
    fn next_up(mut self) -> StatusAnd<Self> {
        ...
        match self.category {
            ...
            Category::Normal => {
                ...
                let sig_mask = (Limb::ONE << (S::PRECISION - 1)) - Limb::ONE;
                ...
                if self.sign {
                    ...
                    let crossing_binade_boundary =
                        self.exp != S::MIN_EXP && self.sig[0] & sig_mask == Limb::ZERO;
    ...
    fn to_u256_r(self, width: usize, round: Round, is_exact: &mut bool) -> StatusAnd<u256> {
        ...
        *is_exact = false;
        ...
        match self.category {
            Category::NaN => Status::INVALID_OP.and(u256::ZERO),
    ...
    fn get_exact_inverse(self) -> Option<Self> {
        ...
        if self.sig != [Limb::ONE << (S::PRECISION - 1)] {
            return None;
        }

        // Get the inverse.
        let mut reciprocal = Self::from_u256(u256::ONE).value;
        let status;
        ...
        assert_eq!(reciprocal.sig, [Limb::ONE << (S::PRECISION - 1)]);
...
impl<S: Semantics> IeeeFloat<S> {
    ...
    fn normalize(mut self, round: Round, mut loss: Loss) -> StatusAnd<Self> {
        ...
        if self.round_away_from_zero(round, loss, 0) {
            ...
            assert_eq!(sig::increment(&mut self.sig), Limb::ZERO);
    ...
    fn from_hexadecimal_string(s: &str, round: Round) -> Result<StatusAnd<Self>, ParseError> {
        ...
        for (p, c) in s.char_indices() {
            ...
            if c == '.' {
                ...
                bit_pos -= 4;
                if bit_pos >= 0 {
                    r.sig[0] |= Limb::from(hex_value) << (bit_pos as usize);
    ...
    fn from_decimal_string(s: &str, round: Round) -> Result<StatusAnd<Self>, ParseError> {
        ...
        loop {
            let mut val = Limb::ZERO;
            let mut multiplier = Limb::ONE;
        ...
        let (pow5_full, mut pow5_calc, mut sig_calc, mut sig_scratch_calc) = {
            ...
            let mut r_scratch: Vec<Limb> = vec![];
        ...
        loop {
            ...
            let calc_normal_from_limbs = |sig: &mut Vec<Limb>,
                                          limbs: &[Limb]|
             -> StatusAnd<ExpInt> {
                sig.resize(limbs_for_bits(calc_precision), Limb::ZERO);
                ...
                if loss == Loss::MoreThanHalf || loss == Loss::ExactlyHalf && sig::get_bit(sig, 0) {
                    // We should never overflow.
                    assert_eq!(sig::increment(sig), Limb::ZERO);
                    omsb = sig::omsb(sig);
            ...
            let half_ulp_err1 = Limb::from(status != Status::OK);
            let (calc_loss, half_ulp_err2);
            if dec_exp >= 0 {
                exp += pow5_exp;

                sig_scratch_calc.resize(sig_calc.len() + pow5_calc.len(), Limb::ZERO);
                calc_loss = sig::mul(
                ...
                half_ulp_err2 = Limb::from(pow5_status != Status::OK);
            } else {
                exp -= pow5_exp;

                sig_scratch_calc.resize(sig_calc.len(), Limb::ZERO);
                calc_loss = sig::div(
                    &mut sig_scratch_calc,
                    &mut exp,
                    &mut sig_calc,
                ...
                half_ulp_err2 =
                    Limb::from(pow5_status != Status::OK || calc_loss != Loss::ExactlyZero) * 2u8;
            }
            ...
            let half_ulp_err = if (half_ulp_err1 + half_ulp_err2).is_zero() {
                inexact * 2u8 // <= inexact half-ulps.
            } else {
                inexact + (half_ulp_err1 + half_ulp_err2) * 2u8
            };
            ...
            let ulps_from_boundary = {
                ...
                let limb = sig_calc[i] & (Limb::MAX >> (LIMB_BITS - 1 - bits % LIMB_BITS));
                let boundary = match round {
                    Round::NearestTiesToEven | Round::NearestTiesToAway => {
                        Limb::ONE << (bits % LIMB_BITS)
                    }
                    _ => Limb::ZERO,
                };
                if i == 0 {
                ...
                } else if limb == boundary.wrapping_sub(Limb::ONE) {
                    if sig_calc[1..i].iter().any(|&x| x != Limb::MAX) {
                        Limb::MAX // A lot.
                    } else {
            ...
            if ulps_from_boundary.saturating_mul(2u8) >= half_ulp_err {
                let mut r = IeeeFloat {
                    sig: [Limb::ZERO],
                    exp,
                    category: Category::Normal,
...
impl Loss {
    ...
    fn through_truncation(limbs: &[Limb], bits: usize) -> Loss {
        ...
        let half = Limb::ONE << (half_bit % LIMB_BITS);
        let has_half = half_limb & half != Limb::ZERO;
        let has_rest = half_limb & (half - Limb::ONE) != Limb::ZERO || !sig::is_all_zeros(rest);
...
mod sig {
    ...
    pub(super) fn get_bit(limbs: &[Limb], bit: usize) -> bool {
        limbs[bit / LIMB_BITS] & (Limb::ONE << (bit % LIMB_BITS)) != Limb::ZERO
    }

    /// Set the given bit.
    pub(super) fn set_bit(limbs: &mut [Limb], bit: usize) {
        limbs[bit / LIMB_BITS] |= Limb::ONE << (bit % LIMB_BITS);
    }
    ...
    pub(super) fn extract(dst: &mut [Limb], src: &[Limb], src_bits: usize, src_lsb: usize) {
        ...
        if n < src_bits {
            let mask = (Limb::ONE << (src_bits - n)) - Limb::ONE;
            dst[dst_limbs - 1] |= (src[dst_limbs] & mask) << (n % LIMB_BITS);
        } else if n > src_bits && src_bits % LIMB_BITS > 0 {
            dst[dst_limbs - 1] &= (Limb::ONE << (src_bits % LIMB_BITS)) - Limb::ONE;
        }
    ...
    pub(super) fn each_chunk<F: FnMut(Limb) -> Limb>(limbs: &mut [Limb], bits: usize, mut f: F) {
        ...
        for limb in limbs.iter_mut().rev() {
            ...
            for i in (0..LIMB_BITS / bits).rev() {
                r |= f((*limb >> (i * bits)) & ((Limb::ONE << bits) - Limb::ONE)) << (i * bits);
            }
            *limb = r;
        }
    }
    ...
    pub(super) fn add(a: &mut [Limb], b: &[Limb], mut c: Limb) -> Limb {
        ...
        for (a, &b) in a.iter_mut().zip(b) {
            ...
            *a = r;
            c = Limb::from(overflow | overflow2);
        }
    ...
    pub(super) fn add_or_sub(
        ...
        if *a_sign ^ b_sign {
            ...
            if reverse {
                ...
                assert_eq!(sub(b_sig, a_sig, borrow), Limb::ZERO);
                a_sig.copy_from_slice(b_sig);
                *a_sign = !*a_sign;
            } else {
                // The code above is intended to ensure that no borrow is necessary.
                assert_eq!(sub(a_sig, b_sig, borrow), Limb::ZERO);
            }
            ...
            assert_eq!(add(a_sig, b_sig, Limb::ZERO), Limb::ZERO);
            loss
        }
    }
    ...
    pub(super) fn widening_mul(a: Limb, b: Limb) -> [Limb; 2] {
        let mut wide = [Limb::ZERO, Limb::ZERO];
        ...
        let select = |limb, i| (limb >> (i * HALF_BITS)) & ((Limb::ONE << HALF_BITS) - Limb::ONE);
        for i in 0..2 {
            for j in 0..2 {
                let mut x = [select(a, i) * select(b, j), Limb::ZERO];
                shift_left(&mut x, &mut 0, (i + j) * HALF_BITS);
                assert_eq!(add(&mut wide, &x, Limb::ZERO), Limb::ZERO);
            }
        }
    ...
    pub(super) fn div(
        ...
        *exp += bits as ExpInt;
        ...
        macro_rules! try_short_div {
            ($W:ty, $H:ty, $half:expr) => {
                if divisor_bits * 2 <= $half {
                    ...
                    let divisor = divisor[0].low_u64() as $H as $W;

                    // Shift the dividend to produce a quotient with the unit bit set.
                    let top_limb = *dividend.last().unwrap();
                    let mut rem = (top_limb >> (LIMB_BITS - (divisor_bits - 1))).low_u64() as $H;
                    ...
                    each_chunk(dividend, $half, |chunk| {
                        let chunk = chunk.low_u64() as $H;
                        let combined = ((rem as $W) << $half) | (chunk as $W);
                        rem = (combined % divisor) as $H;
                        Limb::from((combined / divisor) as $H)
                    });
                    quotient.copy_from_slice(dividend);

                    return lost_fraction(&[Limb::from(rem) << 1], &[Limb::from(divisor)]);
                }
            };
        }
        ...
        for bit in (0..precision).rev() {
            if cmp(dividend, divisor) != Ordering::Less {
                sub(dividend, divisor, Limb::ZERO);
