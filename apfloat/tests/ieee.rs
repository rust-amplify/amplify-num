...
fn next() {
    ...
    // nextUp(+inf) = +inf.
    let test = unpack!(status=, Oct::INFINITY.next_up());
    let expected = Oct::INFINITY;
    ...
    let test = unpack!(status=, Oct::INFINITY.next_down());
    let expected = Oct::largest();
    assert_eq!(status, Status::OK);
    assert!(!test.is_negative());
    assert!(test.bitwise_eq(expected));
    ...
    // nextUp(largest) = +inf
    let test = unpack!(status=, Oct::largest().next_up());
    let expected = Oct::INFINITY;
    assert_eq!(status, Status::OK);
    assert!(test.is_infinite() && !test.is_negative());
    assert!(test.bitwise_eq(expected));
    ...
    //                        = -(-largest + inc)
    //                        = largest - inc.
    let test = unpack!(status=, Oct::largest().next_down());
    let expected = "0x1.ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffep+262143"
        .parse::<Oct>()
        .unwrap();
    assert_eq!(status, Status::OK);
    assert!(!test.is_infinite() && !test.is_negative());
    ...
    let test = unpack!(status=, "0x1p-262378"
        .parse::<Oct>()
        .unwrap()
        .next_up());
    let expected = "0x2p-262378".parse::<Oct>().unwrap();
    assert_eq!(status, Status::OK);
    assert!(test.bitwise_eq(expected));
    ...
    // nextUp(qNaN) = qNaN
    let test = unpack!(status=, Oct::qnan(None).next_up());
    let expected = Oct::qnan(None);
    assert_eq!(status, Status::OK);
    assert!(test.bitwise_eq(expected));

    // nextDown(qNaN) = qNaN
    let test = unpack!(status=, Oct::qnan(None).next_down());
    let expected = Oct::qnan(None);
    assert_eq!(status, Status::OK);
    assert!(test.bitwise_eq(expected));

    // nextUp(sNaN) = qNaN
    let test = unpack!(status=, Oct::snan(None).next_up());
    let expected = Oct::qnan(None);
    assert_eq!(status, Status::INVALID_OP);
    assert!(test.bitwise_eq(expected));

    // nextDown(sNaN) = qNaN
    let test = unpack!(status=, Oct::snan(None).next_down());
    let expected = Oct::qnan(None);
    assert_eq!(status, Status::INVALID_OP);
    assert!(test.bitwise_eq(expected));

    // nextUp(+0) = +smallest
    let test = unpack!(status=, Oct::ZERO.next_up());
    let expected = Oct::SMALLEST;
    assert_eq!(status, Status::OK);
    assert!(test.bitwise_eq(expected));

    // nextDown(+0) = -nextUp(-0) = -smallest
    let test = unpack!(status=, Oct::ZERO.next_down());
    let expected = -Oct::SMALLEST;
    assert_eq!(status, Status::OK);
    assert!(test.bitwise_eq(expected));
    ...
    // nextUp(+Largest Denormal) -> +Smallest Normal.
    let test = unpack!(status=, "0x0.fffffffffffffffffffffffffffffffffffffffffffffffffffffffffffp-262142"
        .parse::<Oct>()
        .unwrap()
        .next_up());
    let expected = "0x1p-262142".parse::<Oct>().unwrap();
    assert_eq!(status, Status::OK);
    ...
    // nextUp(-Smallest Normal) -> -Largest Denormal.
    let test = unpack!(status=, "-0x1p-262142"
        .parse::<Oct>()
        .unwrap()
        .next_up());
    let expected = "-0x0.fffffffffffffffffffffffffffffffffffffffffffffffffffffffffffp-262142"
        .parse::<Oct>()
        .unwrap();
    assert_eq!(status, Status::OK);
    assert!(test.is_denormal());
    ...
    // nextUp(-Normal Binade Boundary) -> -Normal Binade Boundary + 1.
    let test = unpack!(status=, "-0x1p+1".parse::<Oct>().unwrap().next_up());
    let expected = "-0x1.fffffffffffffffffffffffffffffffffffffffffffffffffffffffffffp+0"
        .parse::<Oct>()
        .unwrap();
    assert_eq!(status, Status::OK);
    assert!(test.bitwise_eq(expected));
    ...
    // nextDown(+Normal Binade Boundary) -> +Normal Binade Boundary - 1.
    let test = unpack!(status=, "0x1p+1".parse::<Oct>().unwrap().next_down());
    let expected = "0x1.fffffffffffffffffffffffffffffffffffffffffffffffffffffffffffp+0"
        .parse::<Oct>()
        .unwrap();
    assert_eq!(status, Status::OK);
    ...
    // nextUp(+Normal Binade Boundary - 1) -> +Normal Binade Boundary.
    let test = unpack!(status=, "0x1.fffffffffffffffffffffffffffffffffffffffffffffffffffffffffffp+0"
        .parse::<Oct>()
        .unwrap()
        .next_up());
    let expected = "0x1p+1".parse::<Oct>().unwrap();
    assert_eq!(status, Status::OK);
    assert!(test.bitwise_eq(expected));
    ...
    let test = unpack!(status=, "-0x0.fffffffffffffffffffffffffffffffffffffffffffffffffffffffffffp-262142"
        ...
        .unwrap()
        .next_up());
    let expected = "-0x0.ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffep-262142"
        .parse::<Oct>()
        .unwrap();
    assert_eq!(status, Status::OK);
    assert!(test.is_denormal());
    ...
    let test = unpack!(status=, "0x1p-262142"
        ...
        .unwrap()
        .next_up());
    let expected = "0x1.00000000000000000000000000000000000000000000000000000000001p-262142"
        .parse::<Oct>()
        .unwrap();
    assert_eq!(status, Status::OK);
    assert!(!test.is_denormal());
    ...
    // nextDown(-Smallest Normal) -> -Smallest Normal - inc.
    let test = unpack!(status=, "-0x1.00000000000000000000000000000000000000000000000000000000000p-262142"
        .parse::<Oct>()
        .unwrap()
        .next_down());
    let expected = "-0x1.00000000000000000000000000000000000000000000000000000000001p-262142"
    ...
    // nextUp(-0x1p-16381) -> -0x1.ffffffffffffffffffffffffffffp-16382
    let test = unpack!(status=, "-0x1p-262141".parse::<Oct>().unwrap().next_up());
    let expected = "-0x1.fffffffffffffffffffffffffffffffffffffffffffffffffffffffffffp-262142"
        .parse::<Oct>()
        .unwrap();
    assert_eq!(status, Status::OK);
    assert!(test.bitwise_eq(expected));
    ...
    //         -0x1p-16381
    let test = unpack!(status=, "-0x1.fffffffffffffffffffffffffffffffffffffffffffffffffffffffffffp-262142"
        .parse::<Oct>()
        .unwrap()
        .next_down());
    let expected = "-0x1p-262141".parse::<Oct>().unwrap();
    assert_eq!(status, Status::OK);
    assert!(test.bitwise_eq(expected));
    ...
    let test = unpack!(status=, "0x1p-262141".parse::<Oct>().unwrap().next_down());
    let expected = "0x1.fffffffffffffffffffffffffffffffffffffffffffffffffffffffffffp-262142"
        .parse::<Oct>()
        .unwrap();
    assert_eq!(status, Status::OK);
    ...
    // nextUp(+Denormal) -> +Denormal.
    let test = unpack!(status=, "0x0.fffffffffffffffffffffffffffffffffffffffffffffffffffffff000cp-262142"
        .parse::<Oct>()
        .unwrap()
        .next_up());
    let expected = "0x0.fffffffffffffffffffffffffffffffffffffffffffffffffffffff000dp-262142"
        .parse::<Oct>()
        .unwrap();
    assert_eq!(status, Status::OK);
    assert!(test.is_denormal());
    ...
    let test = unpack!(status=, "0x0.fffffffffffffffffffffffffffffffffffffffffffffffffffffff000cp-262142"
        ...
        .unwrap()
        .next_down());
    let expected = "0x0.fffffffffffffffffffffffffffffffffffffffffffffffffffffff000bp-262142"
        .parse::<Oct>()
        .unwrap();
    assert_eq!(status, Status::OK);
    assert!(test.is_denormal());
    ...
    // nextUp(+Normal) -> +Normal.
    let test = unpack!(status=, "0x1.fffffffffffffffffffffffffffffffffffffffffffffffffffffff000cp-260000"
        .parse::<Oct>()
        .unwrap()
        .next_up());
    let expected = "0x1.fffffffffffffffffffffffffffffffffffffffffffffffffffffff000dp-260000"
        .parse::<Oct>()
        .unwrap();
    assert_eq!(status, Status::OK);
    assert!(!test.is_denormal());
    ...
    let test = unpack!(status=, "0x1.fffffffffffffffffffffffffffffffffffffffffffffffffffffff000cp-260000"
        ...
        .unwrap()
        .next_down());
    let expected = "0x1.fffffffffffffffffffffffffffffffffffffffffffffffffffffff000bp-260000"
        .parse::<Oct>()
        .unwrap();
    assert_eq!(status, Status::OK);
    assert!(!test.is_denormal());
...
fn fma() {
    ...
    {
        let mut m1 = X87DoubleExtended::from_u256(1u8.into()).value;
        let m2 = X87DoubleExtended::from_u256(1u8.into()).value;
        let a = X87DoubleExtended::from_u256(3u8.into()).value;
...
fn denormal() {
    ...
    {
        assert!(!X87DoubleExtended::from_u256(0u8.into()).value.is_denormal());
        ...
        t /= X87DoubleExtended::from_u256(2u8.into()).value;
        assert!(t.is_denormal());
    }

    // Test quadruple precision
    {
        assert!(!Quad::from_u256(0u8.into()).value.is_denormal());
        ...
        t /= Quad::from_u256(2u8.into()).value;
        assert!(t.is_denormal());
    }
}
...
fn to_integer() {
    ...
    assert_eq!(
        Status::OK.and(10u8.into()),
        "10".parse::<Double>()
            .unwrap()
            .to_u256_r(5, Round::TowardZero, &mut is_exact,)
    );
    assert!(is_exact);

    assert_eq!(
        Status::INVALID_OP.and(0u8.into()),
        "-10"
            .parse::<Double>()
            .unwrap()
            .to_u256_r(5, Round::TowardZero, &mut is_exact,)
    ...
    assert_eq!(
        Status::INVALID_OP.and(31u8.into()),
        "32".parse::<Double>()
            .unwrap()
            .to_u256_r(5, Round::TowardZero, &mut is_exact,)
    );
    ...
    assert_eq!(
        Status::OK.and(i256::from(-10)),
        "-10"
            .parse::<Double>()
            .unwrap()
            .to_i256_r(5, Round::TowardZero, &mut is_exact,)
    );
    assert!(is_exact);

    assert_eq!(
        Status::INVALID_OP.and(i256::from(-16)),
        "-17"
            .parse::<Double>()
            .unwrap()
            .to_i256_r(5, Round::TowardZero, &mut is_exact,)
    ...
    assert_eq!(
        Status::INVALID_OP.and(i256::from(15)),
        "16".parse::<Double>()
            .unwrap()
            .to_i256_r(5, Round::TowardZero, &mut is_exact,)
    );
...
#[test]
fn nan() {
    fn nanbits<T: Float>(signaling: bool, negative: bool, fill: u256) -> u256 {
        let x = if signaling {
            T::snan(Some(fill))
        } else {
            T::qnan(Some(fill))
    ...
    assert_eq!(u256::from(0x7fc00000u128), nanbits::<Single>(false, false, 0u128.into()));
    assert_eq!(u256::from(0xffc00000u128), nanbits::<Single>(false, true, 0u128.into()));
    assert_eq!(u256::from(0x7fc0ae72u128), nanbits::<Single>(false, false, 0xae72u128.into()));
    assert_eq!(u256::from(0x7fffae72u128), nanbits::<Single>(false, false, 0xffffae72u128.into()));
    assert_eq!(u256::from(0x7fa00000u128), nanbits::<Single>(true, false, 0u128.into()));
    assert_eq!(u256::from(0xffa00000u128), nanbits::<Single>(true, true, 0u128.into()));
    assert_eq!(u256::from(0x7f80ae72u128), nanbits::<Single>(true, false, 0xae72u128.into()));
    assert_eq!(u256::from(0x7fbfae72u128), nanbits::<Single>(true, false, 0xffffae72u128.into()));

    assert_eq!(u256::from(0x7ff8000000000000u128), nanbits::<Double>(false, false, 0u8.into()));
    assert_eq!(u256::from(0xfff8000000000000u128), nanbits::<Double>(false, true, 0u8.into()));
    assert_eq!(
        u256::from(0x7ff800000000ae72u128),
        nanbits::<Double>(false, false, 0xae72u128.into())
    );
    assert_eq!(
        u256::from(0x7fffffffffffae72u128),
        nanbits::<Double>(false, false, 0xffffffffffffae72u128.into())
    );
    assert_eq!(u256::from(0x7ff4000000000000u128), nanbits::<Double>(true, false, 0u8.into()));
    assert_eq!(u256::from(0xfff4000000000000u128), nanbits::<Double>(true, true, 0u8.into()));
    assert_eq!(
        u256::from(0x7ff000000000ae72u128),
        nanbits::<Double>(true, false, 0xae72u128.into())
    );
    assert_eq!(
        u256::from(0x7ff7ffffffffae72u128),
        nanbits::<Double>(true, false, 0xffffffffffffae72u128.into())
    );
}
...
fn zero() {
    ...
    fn test<T: Float>(sign: bool, bits: u256) {
    ...
    test::<Half>(false, u256::ZERO);
    test::<Half>(true, u256::from(0x8000u128));
    test::<Single>(false, u256::ZERO);
    test::<Single>(true, u256::from(0x80000000u128));
    test::<Double>(false, u256::ZERO);
    test::<Double>(true, u256::from(0x8000000000000000u128));
    test::<Quad>(false, u256::ZERO);
    test::<Quad>(true, u256::from(0x8000000000000000_0000000000000000u128));
    test::<X87DoubleExtended>(false, u256::ZERO);
    test::<X87DoubleExtended>(true, u256::from(0x8000_0000000000000000u128));
}
...
fn scalbn() {
    ...
    let payload = (u256::ONE << 50) | (u256::ONE << 49) | (u256::from(1234u32) << 32) | u256::ONE;
    ...
    assert_eq!(payload, quiet_payload.to_bits() & ((u256::ONE << 51) - u256::ONE));
...
fn frexp() {
    ...
    assert_eq!(IEK_NAN, exp);
    assert!(frac.is_nan() && !frac.is_signaling());
    assert_eq!(payload, frac.to_bits() & ((u256::ONE << 51) - u256::ONE));
